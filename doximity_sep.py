...
INPUT_FILE = "doximity.csv"
OUTPUT_FILE = "doximity_output.csv"
DETAILS_OUTPUT_FILE = "doximity_output_details.csv"
SCREENSHOT_ROOT_DIR = r"C:\Users\91630\Downloads\Screenshots"
ERROR_LOG_FILE = "error.log"
WAIT_SECONDS = 30
PAGE_SLEEP = 0.5
HEADLESS = True

# Retry policy for a failed scrape. A record only counts as valid when a name
# is captured; if not, we retry a few times before giving up (and never save it).
MAX_ATTEMPTS = 2
RETRY_BACKOFF = 2.0
...
def setup_logging():
    ...
    # No file handler: errors are not persisted to error.log to keep disk/CPU I/O low.
...
def build_driver():
    ...
    # CPU-reduction flags: turn off GPU/rasterizer work, background timers,
    # extensions, and the many helper processes that keep the CPU busy.
    ...
    options.add_argument("--disable-features=Translate,MediaRouter,OptimizationHints,site-per-process")
...
class ProfileNameMissing(Exception):
    """Raised when a profile loads but no name can be parsed -> treated as failure."""
...
def scrape_profile(driver, profile_url: str, source_name: str):
    ...
    # Name is the required field. No name -> failure; bail before the costly
    # screenshot so nothing partial gets written.
    if not (profile_data.get("first_name") or profile_data.get("profile_name")):
        raise ProfileNameMissing(f"no name captured for {profile_url}")
                ...
                for index, profile_url in enumerate(profile_urls, start=1):
                    ...
                    for attempt in range(1, MAX_ATTEMPTS + 1):
                        try:
                            ...
                            break
                        except (TimeoutException, WebDriverException, ProfileNameMissing, Exception) as exc:
                            print(f"    attempt {attempt}/{MAX_ATTEMPTS} failed: {exc}")
                            if attempt < MAX_ATTEMPTS:
                                time.sleep(RETRY_BACKOFF)

                    if row_data is None:
                        # Name could not be captured after retries -> failure, do not save.
                        # Left out of processed_profile_urls so a later re-run can retry it.
                        print(f"    no valid record (name missing); skipping (not saved): {profile_url}")
                        continue
