...
def build_driver(driver_path=None):
...
def worker(thread_name, keywords, zip_codes, driver_path, stats, stats_lock):
...
def main():
    ...
    # Resolve the chromedriver once so the worker threads don't race installing it.
    ...
    for index, group in enumerate(keyword_groups, start=1):
        thread = threading.Thread(
            target=worker,
            args=(f"Thread-{index}", group, zip_codes, driver_path, stats, stats_lock),
            name=f"Thread-{index}",
        )
        thread.start()
